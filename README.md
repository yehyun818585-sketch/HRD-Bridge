...
`Next.js` · `Supabase` · `Vercel` · `Resend` · `pdf-parse`· `server-only`
...
## 해결 방안 
> **공단 시스템 제출 전, 기업의 제출 상태를 센터가 사전 확인(정보 미러링) & 상태·서류·소통의 단일 기준(SSOT)이 되는 화면**
...
- **③** **기준 서식**과 비교해 정의된 필수 항목이 본문에 존재하는지 대조
...
센터는 여러 기업 데이터를 열람하므로 코드 관문을, 기업은 센터를 통해 받은 초대 경로를 통해서만 가입 가능합니다. 
...
* **초대·코드 기반 가입**
  센터가 기업 담당자 이메일을 입력하면 초대 메일이 발송되고, 수락 시 role·company_id가 자동 부여됩니다. 센터 가입 코드는 서버에서만 비교되어 브라우저에 노출되지 않습니다.
...
* **승인/반려 워크플로우 & 실시간 미러링**
  ...
  **상태 자동 감지**
  - 준비중: 필수서류 하나라도 미첨부(사업계획서·전담인력등록) 
...
flowchart LR
    ...
    subgraph VC["Vercel (배포)"]
        subgraph NX["Next.js 16 · TypeScript"]
            FE["화면 (App Router)"]
            BE["API Routes<br/>가입코드 검증 · 초대 · 서류검증 · 알림"]
        end
        SO["server-only<br/>service_role 키의<br/>클라이언트 노출을 빌드 타임 차단"]
    end

    subgraph SB["Supabase"]
        AUTH["Auth<br/>초대·코드 기반 가입"]
        DB[("Postgres + RLS<br/>센터·기업 데이터 격리")]
        RT["Realtime<br/>승인·반려·댓글 즉시 반영"]
    end

    PP["pdf-parse<br/>PDF 내용 추출 →<br/>확인됨·누락·불일치 판별"]
    GEN["docx · pdfkit<br/>양식·데모 서류 생성"]
    RS["Resend<br/>초대·알림 메일"]
...
| 기술 | 용도 / 선택 이유 |
|---|---|
| **Next.js 16** (App Router, TypeScript) | 1인 개발로, 화면(프론트엔드)과 백엔드(API 라우트)를 하나의 프로젝트로 다루는 풀스택 프레임워크 선택|
| **Supabase** (Auth/RLS/DB/Realtime) | 데이터를 저장하는 DB(회사정보, 서류상태 등), DB차원에서 RLS를 통한 내부 통제 설계 가능. 승인·댓글의 실시간 반영(realtime)|
| **Vercel** | github 변경시 프로젝트 자동 배포됨. 환경변수로 센터 가입 코드·service_role 키를 서버 전용으로 관리 |
| **Resend** | supabase 이메일 발송의 한계로, 발송만 resend 이용 |
...
배포된 서비스를 직접 점검하는 과정에서 접근통제 결함을 발견하고, **발견 → 원인 분석(접근허용) → 통제 설계(RLS+server only) → 검증** 순으로 조치했습니다.
 
* **화면 통제의 한계 확인:** 로그인 없이 URL 직접 입력(화면 통제 우회)만으로 전체 기업 데이터가 조회되는 것을 시크릿창 테스트로 확인
...
* **server-only:** service_role 키를 사용하는 서버 전용 모듈이 실수로 클라이언트 코드에 import되면 빌드 자체가 실패하도록 강제(예방통제) 
